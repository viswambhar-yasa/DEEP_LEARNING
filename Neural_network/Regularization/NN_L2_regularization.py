...
def cost_function(A, Y,parameters,lamdb=0):
    m = Y.size
    ...
    for i in range(L):
        wl += np.sum(np.square(parameters['w'+str(i+1)]))
    L2_regu=(lamdb/(2*m))*wl
    cost+=L2_regu
...
def backward_propagation(AL, Y, cache,lambd=0, typ='relu'):
    grad = {}
    n_layer = len(cache)
    #print(n_layer)
    m = AL.shape[1]
    ...
    dA_prev_temp, dW_temp, db_temp = linear_backward_regu(dZ, linear_cache, lambd)
...
def NN_deep_layered(X, Y, layer_dim, num_iter=1000, learning_rate=0.05,lamdb=0, print_cost=False,p_ty='ranm'):
    np.random.seed(1)
    parameters = {}
    NN_cost = []
