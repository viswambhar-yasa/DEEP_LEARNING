...
def forward_propagation(X, parameters,ke_prob=1, typ='relu'):
    np.random.seed(1)
    fw_cache = []
    ...
    for i in range(1, n_layer):
        ...
        D=(D<ke_prob).astype(int)
        A=A*D
        A = A/ke_prob
        ac_cache=(Z,D,A)
        cache = (ln_cache, ac_cache)
...
def linear_backward_regu(dZ, cache, lambd):
    A_prev, W, b = cache
    m = A_prev.shape[1]
...
def backward_propagation(AL, Y, cache, ke_prob, typ='relu'):
    grad = {}
    n_layer = len(cache)
    #print(n_layer)
    m = AL.shape[1]
    ...
    Z,DL,AL = activation_cache
    #derivative of activation function
    #dZ= np.multiply(dAL, np.int64(AL > 0))
    dZ = der_activation_fun(dAL, activation_cache, typ='sig')
    #derivatives of parameters in that hidden layer
    dA_prev_temp, dW_temp, db_temp = linear_backward_regu(dZ, linear_cache, ke_prob)
    
    grad["dA" + str(n_layer-1)] = dA_prev_temp
    grad["dZ" + str(n_layer)] = dZ
...
def NN_deep_layered(X, Y, layer_dim, num_iter=1000, learning_rate=0.05, ke_prob=1, print_cost=False, p_ty='ranm'):
    np.random.seed(1)
    parameters = {}
    NN_cost = []
    ...
    for i in range(num_iter):
        ...
        AL, fw_cache = forward_propagation(A0, parameters, ke_prob)
        #print(AL)
        ...
        grad = backward_propagation(AL, Y, fw_cache, ke_prob)
        #print(grad)
...
def predict(X, Y, parameters, tol=0.5):
    ...
    p = np.zeros((1, m))
    # Forward propagation
    probas, fw_caches = forward_propagation(X, parameters, ke_prob=1)
    # Activation function
